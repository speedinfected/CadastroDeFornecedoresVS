...

{
...

    {
        ...
        private readonly HashSet<string> _estadosValidos = new HashSet<string>
{
    ...
    "RJ", "RN", "RS", "RO", "RR", "SC", "SP", "SE", "TO"
...

        {
...

            {
                ...
                MessageBox.Show($"Erro ao carregar dados do fornecedor: {ex.Message}", "Erro",
...

        {
            ...
            catch (ApplicationException ex) // Captura nossa exceção customizada
            {
                MessageBox.Show(ex.Message, "CNPJ Duplicado",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        // Método genérico para tratar o evento TextChanged de todos os campos
...
if (txtResponsavel != null) camposObrigatorios.Add(txtResponsavel, "Informe o Nome do Responsável.");
            ...
            foreach (var campo in camposObrigatorios)
            {
                if (string.IsNullOrWhiteSpace(campo.Key.Text))
                {
                    ...
                    if (camposValidos) // Só mostra a mensagem para o primeiro erro encontrado
                    {
        ...
        public static class CNPJValidator
        {
            ...
            {
                cnpj = new string(cnpj.Where(char.IsDigit).ToArray());
                return cnpj.Length == 14; // Valida apenas o tamanho (implemente a validação real se necessário)
            }
        }
